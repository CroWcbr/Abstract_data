...
bool test_all()
{
	...
	for (int i = 0; i < T_COUNT; ++i)
	{
		...
		int pos_splice_end = rand() % ft_test[i].size();
		if (pos_splice_start > pos_splice_end) 
			std::swap(pos_splice_start, pos_splice_end);
		...
		typename FT::iterator ft_it_splice_end = ft_test_splice[i].begin();
		typename STD::iterator std_it_splice_end = std_test_splice[i].begin();
		ft::advance(ft_it_splice_start, pos_splice_start);
		std::advance(std_it_splice_start, pos_splice_start);
		ft::advance(ft_it_splice_end, pos_splice_end);
		std::advance(std_it_splice_end, pos_splice_end);

		ft_test[i].splice(ft_it_tmp, ft_test_splice[i], ft_it_splice_start, ft_it_splice_end);
		std_test[i].splice(std_it_tmp, std_test_splice[i], std_it_splice_start, std_it_splice_end);
...
void	test_time(bool leaks, time_t& start_ft, time_t& start_std, time_t& end_ft, time_t& end_std)
{
	...
	int array_iter_pos_splice_end[T_COUNT];
	for (int i = 0; i < T_COUNT; ++i)
	{
		if (ft_test[i].empty())
		{
			...
			array_iter_pos_splice_end[i] = -1;
		}
		else
		{
			array_iter_pos[i] = rand() % ft_test[i].size();
			array_iter_pos_splice_start[i] = rand() % ft_test[i].size();
			array_iter_pos_splice_end[i] = rand() % ft_test[i].size();
			if (array_iter_pos_splice_start[i] > array_iter_pos_splice_end[i]) 
				std::swap(array_iter_pos_splice_start[i], array_iter_pos_splice_end[i]);
		}
	}
	...
	if (!leaks)
	{
		...
		typename STD::iterator std_it_splice_end;
		for (int i = 0; i < T_COUNT; ++i)
		{
			...
			std_it_splice_end = std_test_splice[i].begin();
			std::advance(std_it_splice_end, array_iter_pos_splice_end[i]);
	...
	typename FT::iterator ft_it_splice_end;
	if (!leaks)
		start_ft = timer();
	for (int i = 0; i < T_COUNT; ++i)
	{
		...
		ft_it_splice_end = ft_test_splice[i].begin();
		ft::advance(ft_it_splice_end, array_iter_pos_splice_end[i]);
