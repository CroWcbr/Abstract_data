...
bool test_all()
{
	...
	for (int i = 0; i < T_COUNT; ++i)
	{
		...
		typename FT::iterator ft_it = ft_test[i].end();
		typename STD::iterator std_it = std_test[i].end();

		if (*(--ft_it) != *(--std_it))
			return false;

		typename FT::const_iterator ft_it_const = ft_test[i].end();
		typename STD::const_iterator std_it_const = std_test[i].end();
