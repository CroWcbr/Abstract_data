...
bool test_all()
{
	...
	for (int i = 0; i < T_COUNT; ++i)
	{
		...
		typename FT::iterator ft_it = ft_test[i].upper_bound(elem);
		typename STD::iterator std_it = std_test[i].upper_bound(elem);
		...
		typename FT::const_iterator ft_it_const = ft_test[i].upper_bound(elem);
		typename STD::const_iterator std_it_const = std_test[i].upper_bound(elem);
...
void	test_time(bool leaks, time_t& start_ft, time_t& start_std, time_t& end_ft, time_t& end_std)
{
	...
	if (!leaks)
	{
		...
		for (int i = 0; i < T_COUNT; ++i)
		{
			std_test[i].upper_bound(array_elem[i]);
		}
		end_std = timer();
	}
	...
	for (int i = 0; i < T_COUNT; ++i)
	{
		ft_test[i].upper_bound(array_elem[i]);
	}
	if (!leaks)
		end_ft = timer();
}
