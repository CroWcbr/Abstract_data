...
void	printTime(time_t& start_ft, time_t& start_std, time_t& end_ft, time_t& end_std)
{
	...
	if (t_std + delta_time >= t_ft || t_std * 5 >= t_ft)
		printElement(GREEN + time_to_string(t_ft) + "ms" + RESET);
	else if (t_std * 20 >= t_ft)
		printElement(YELLOW + time_to_string(t_ft) + "ms" + RESET);
	else
		printElement(RED + time_to_string(t_ft) + "ms" + RESET);

	if (t_ft + delta_time >= t_std || t_ft * 5 >= t_std)
		printElement(GREEN + time_to_string(t_std) + "ms" + RESET);
	else if (t_ft * 20 >= t_std)
		printElement(YELLOW + time_to_string(t_std) + "ms" + RESET);
	else
