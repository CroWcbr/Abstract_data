...
#include <iostream>
	...
	#if TYPE == 1
		#include <set>
		#define	TEST_CONTAINER	std::multiset
		#define	NAMESPACE		std
	#elif TYPE == 2
		#include "../src/containers/set.hpp"
		#define	TEST_CONTAINER	ft::multiset
		#define	NAMESPACE		ft
	#endif
#endif
...
int main()
{
	...
	// assert(*v0.erase(v0.begin()) == 'b' && v0.size() == 8);	// ISO+IEC+14882-1998 - erase is void, but in stl return iterator
	v0.erase(v0.begin());										// the same previos
	assert(*v0.begin() == 'b' && v0.size() == 8);				// the same previos
	// assert(*v0.erase(v0.begin(), ++v0.begin()) == 'c' && v0.size() == 7);	// ISO+IEC+14882-1998 - erase is void, but in stl return iterator
	v0.erase(v0.begin(), ++v0.begin());											// the same previos
	assert(*v0.begin() == 'c' && v0.size() == 7);								// the same previos
